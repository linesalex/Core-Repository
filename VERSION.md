...
### 🗺️ **Network Map Export: Geographic Layout & Line-by-Line Route Schedule**

Production-scale feedback on the v3.5.0 PDF Network Map Export showed the force-directed diagram wasn't clear or geographically sensible with real data, and the Route Schedule's grid table was truncating long carrier/UCN text. Both are reworked:

- **Nodes are now positioned by real-world geography instead of pure physics.** Every POP is resolved to an approximate latitude/longitude - first from `location_reference.latitude`/`longitude` if ever populated, otherwise automatically from its city/country using the offline world-cities database (`all-the-cities`, already a backend dependency) - so a diagram of EMEA genuinely reads like a map: London and Paris sit west of Frankfurt, Milan sits south of both, etc., instead of wherever force-directed physics happened to settle
- **Same-city POPs (e.g. four Newark POPs) are arranged as a small local cluster** around that city's map position, rather than every node fighting for the exact same point
- **Diagrams now use the full shared page instead of leaving large blank margins.** Every page in an export shares one physical size (a Puppeteer constraint), sized by the tallest content anywhere in the document - which, once diagrams are placed geographically, is usually a page group's Route Schedule/POP Reference rather than its own diagram. A diagram smaller than that shared canvas is now automatically re-laid-out to actually fill it, instead of sitting in a small corner of mostly-blank space
- **Route Schedule is now one full-text line per route** ("12. IPCASH1 ↔ IPCASH2   UCN ASHASH2-HOU-100G-001   •   Latency 0.25ms   •   Bandwidth 100 Gb   •   Carrier Digital Realty") instead of a narrow grid table - long carrier and UCN values are never truncated, wrapping onto a second line if needed. Flows across at most 2 columns (down from 3) so each line stays wide enough for full text
- POP Code Reference (address annex) is unchanged - still flows across 2-4 columns based on entry count
- **Follow-up tuning (same day):** production-scale data showed dense clusters (e.g. many EMEA POPs bunched together) still packed circles close enough to make lines hard to trace, even with geographic positioning - the enforced minimum edge-to-edge gap between any two POP circles was doubled twice in a row (26px → 52px → 104px collision padding per node, i.e. a 52px → 104px → 208px minimum gap between any two nodes), with the per-page "geographic proportionality" budget (`naturalWidth`/`naturalHeight` multipliers) pulled back correspondingly each time (380/300 → 300/240 → 225/180) so that extra spacing doesn't just compound into ever-larger PDFs
- Also fixed while testing: `Bangalore` failed to geocode (`all-the-cities` only lists it as `Bengaluru`) - added as an alias
- **Follow-up tuning (same day):** with the doubled spacing making the diagrams clearer, the remaining problem was telling which line was which through a busy crossing - every local route used to draw as an identical flat gray straight line. Each route now draws as a gentle bezier curve (deterministic per POP pair, so two lines that used to sit exactly on top of each other now visually separate) in a distinct, deterministic color (also per POP pair - INTER-regional routes keep their existing purple/dashed look, unchanged), with thickness/opacity scaled to the route's bandwidth (thin+faint under 1Gb, medium 1-10Gb, thick+bold 10Gb+/Dark Fiber) so backbone circuits visually stand out over minor ones. Each numbered tag badge's border now also matches its own line's color, so a badge and its line can be visually paired at a glance
- **Follow-up tuning (same day):** the previous tag-declutter logic pushed a colliding badge in whatever 2D direction cleared the obstacle, which could leave the badge floating just off to one side of its own curve/stub line. Tag placement was reworked so a badge can only ever slide **along** its own line - a local route's badge searches positions along its own bezier curve, a stub's badge searches positions further out along its own straight ray from the node - so every badge now sits exactly on the line it labels, while still guaranteed to never overlap a POP circle or another badge
...
- `backend/checkPdfRender.js` — diagnostic that exercises the real render path (local Chromium or sidecar) and writes a test PDF, for validating a host before wiring PM2
...
- **`d3-force` pinned to `^2.1.1`** (was `^3.0.0`). d3-force 3.x is **ESM-only**, and `require()` of ESM is only supported from Node 22.12 onward, so on the Node 16 production host `require('d3-force')` threw `ERR_REQUIRE_ESM` and — because `routes.js` requires `networkMapRenderer.js` at boot — crashed the entire backend, exactly as the `??=` error had. It stayed hidden until the puppeteer require on the preceding line stopped throwing first. 2.1.1 is the last CommonJS release and exposes an identical API for `forceSimulation`/`forceLink`/`forceManyBody`/`forceCollide`/`forceX`/`forceY`; verified by generating a real multi-page PDF with Node's `--no-experimental-require-module` flag to reproduce Node 16 module resolution.
- **`sqlite3` pinned to exactly `5.0.2`** (was `^5.1.7`) so a clean `npm ci` resolves the version RHEL 7 has actually been running since v3.3.3, instead of a 5.1.x whose prebuilt binary may not match glibc 2.17 and would fall back to compiling — impossible without `gcc-c++`. Verified to install and run on Node 22 as well, so dev and production now share one version. `engines.node` is declared as `>=16.13.2` to document the floor Puppeteer imposes.

Docker CE also cannot install on this host (`container-selinux >= 2:2.74`, `fuse-overlayfs >= 0.7`, `slirp4netns >= 0.4` live in the RHEL 7 extras channel, and the system is unregistered). The sidecar can therefore run on any other Docker/Node ≥18 host with `PDF_RENDER_SIDECAR_URL` pointed at it — optionally secured with the new `PDF_RENDER_SIDECAR_TOKEN` / `SIDECAR_AUTH_TOKEN` pair — **but a container is no longer required at all**: EPEL ships a Chromium *compiled for el7* (glibc 2.17), so pointing `PUPPETEER_EXECUTABLE_PATH` at `/usr/lib64/chromium-browser/headless_shell` renders on the host itself with no Docker, no subscription, and no second machine. Only Chromium's three missing leaf libraries (`flac-libs`, `opus`, `libatomic`) need grafting from the public CentOS 7 vault. Driving a mismatched Chromium with `puppeteer@21.11.0` was validated by rendering successfully against Chrome 148 — the renderer only uses `setContent()` and `page.pdf()`, which are stable across releases. `RHEL_PRODUCTION_DEPLOYMENT_V3.5.0.md` was fully rewritten against the real host state, including the Node 14 → 16.20.2 upgrade (Node 18+ is impossible on RHEL 7: it requires glibc ≥2.28, having moved its build host to RHEL 8).
